...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 13-06-2020 a las 04:10:38
...
DROP PROCEDURE IF EXISTS `sp_borrarItemTemp_wvp`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_borrarItemTemp_wvp` (IN `_id` INT, IN `_idUsuario` INT)  NO SQL
BEGIN

	DELETE FROM temp_movimientos WHERE id_temp = _id;
    
    SELECT t.id_temp, p.id_producto, p.Nombre as producto, b.Nombre as bodega, e.Nombre as estante, n.Nivel as nivel, t.cantidad, t.costo, t.id_orden, t.id_usuario  FROM temp_movimientos t INNER JOIN
...
WHERE t.id_usuario = _idUsuario;

END$$

DROP PROCEDURE IF EXISTS `sp_compraYdespachoOrden_wvp`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_compraYdespachoOrden_wvp` (IN `_com` VARCHAR(100), IN `_idbodega` INT, IN `_idconcepto` INT, IN `_idusuario` INT)  NO SQL
INSERT INTO ordenes (comentario, id_bodega, id_concepto,id_usuario) VALUES (_com, _idbodega, _idconcepto,_idusuario)$$
...
DROP PROCEDURE IF EXISTS `sp_consultaBodegaProductos_wvp`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_consultaBodegaProductos_wvp` ()  NO SQL
SELECT i.Costo_total, b.Nombre as bodega, p.Nombre as producto FROM inventario i INNER JOIN nivel n ON n.id_nivel = i.id_nivel INNER JOIN estante e ON e.id_estante = n.id_estante INNER JOIN bodega b ON b.id_bodega = e.id_bodega INNER JOIN productos p ON p.id_producto = i.id_producto$$
...
DROP PROCEDURE IF EXISTS `sp_consultaConcepto_wvp`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_consultaConcepto_wvp` (IN `_id` INT)  NO SQL
select * from vw_concepto_wvp WHERE id_tipo_concepto = _id$$

DROP PROCEDURE IF EXISTS `sp_consultaCostos_wvp`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_consultaCostos_wvp` ()  NO SQL
SELECT i.Costo_total, p.Nombre as producto FROM inventario i INNER JOIN productos p ON p.id_producto = i.id_producto ORDER BY  i.Costo_total DESC$$

DROP PROCEDURE IF EXISTS `sp_consultaEstanteCP_wvp`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_consultaEstanteCP_wvp` (IN `_id` INT)  NO SQL
SELECT * from vw_estantes_wvp where vw_estantes_wvp.ID = _id$$

DROP PROCEDURE IF EXISTS `sp_consultaEstanteExiste_wvp`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_consultaEstanteExiste_wvp` (IN `_idBodega` INT, IN `_nomEstante` VARCHAR(20))  NO SQL
SELECT * FROM vw_estantes_wvp WHERE id = _idBodega AND estante = _nomEstante$$
...
DROP PROCEDURE IF EXISTS `sp_consultaidbodega`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_consultaidbodega` (IN `_id_bodega` INT(11))  NO SQL
SELECT * FROM vw_bodega_wvp WHERE vw_bodega_wvp.id_bodega = _id_bodega$$
...
DROP PROCEDURE IF EXISTS `sp_consultaInventarioCP_wvp`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_consultaInventarioCP_wvp` (IN `_proc` INT, IN `_nv` INT)  NO SQL
SELECT * FROM vw_inventario_wvp w WHERE w.id_producto = _proc AND w.id_nivel = _nv$$
...
DROP PROCEDURE IF EXISTS `sp_consultaNivelBodegaEstante_wvp`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_consultaNivelBodegaEstante_wvp` (IN `_idBodega` INT, IN `_idEstante` INT)  NO SQL
SELECT * FROM vw_nivel_wvp WHERE vw_nivel_wvp.id_bodega = _idBodega AND vw_nivel_wvp.id_estante = _idEstante$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_consultaproductosCP_de` (IN `_idProc` INT)  NO SQL
SELECT * FROM vw_productos_wvp WHERE id_producto=_idProc$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_entradaMovimiento_wvp` (IN `_cant` DOUBLE(11,2), IN `_cto` DOUBLE(11,2), IN `_idprodc` INT, IN `_idnivel` INT, IN `_idBodega` INT, IN `_idUsuario` INT)  NO SQL
BEGIN

DECLARE num_ord INT;
#call sp_entradaMovimiento_wvp (1,100,2,2,1,1)
call sp_compraYdespachoOrden_wvp ('sp_entradaMovimiento_wvp',_idBodega,1,_idUsuario);
SET num_ord = LAST_INSERT_ID();
#SELECT num_ord; 
INSERT INTO movimientos (Cantidad_entrada,Costo_entrada,id_producto,id_nivel,id_orden) VALUES (_cant,_cto,_idprodc,_idnivel,num_ord);
SELECT i.id_inventario,  i.Cantidad_existencia, i.Costo_uni, i.Costo_total FROM inventario i WHERE i.id_nivel = _idNivel and i.id_producto = _idprodc;
...
DROP PROCEDURE IF EXISTS `sp_insertarTempMovimiento_wvp`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insertarTempMovimiento_wvp` (IN `_idProdc` INT, IN `_idNivel` INT, IN `_cant` DOUBLE, IN `_costo` DOUBLE, IN `_idUsuario` INT, IN `_idOrden` INT)  NO SQL
BEGIN

INSERT INTO temp_movimientos (id_producto,id_nivel,cantidad, costo, id_usuario,id_orden) VALUES (_idProdc, _idNivel, _cant, _costo, _idUsuario, _idOrden); 
...
SELECT t.id_temp, p.id_producto, p.Nombre as producto, b.Nombre as bodega, e.Nombre as estante, n.Nivel as nivel, t.cantidad, t.costo, t.id_orden, t.id_usuario  FROM temp_movimientos t INNER JOIN
    ...
    estante e ON e.id_estante = n.id_estante INNER JOIN
    bodega b ON b.id_bodega = e.id_bodega
WHERE t.id_usuario = _idUsuario AND t.id_orden = _idOrden;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_ModificarEstante_ksn` (IN `_id_estante` INT, IN `_Nombre` VARCHAR(25), IN `_estatus` CHAR(1), IN `_id_bodega` INT)  BEGIN

DECLARE _stt CHAR(1) DEFAULT 'A';
...
#IF NOT EXISTS (SELECT * FROM vw_estantes_wvp v WHERE (v.estante =! _Nombre and v.ID = _id_bodega)) THEN
 
SELECT COUNT(*) INTO _cant FROM vw_estantes_wvp v WHERE (v.estante =! _Nombre and v.ID = _id_bodega AND v.letra_status = _estatus);
IF (_cant = 0) THEN
BEGIN
	#IF NOT EXISTS (SELECT * FROM vw_estantes_wvp v WHERE v.ID = _id_bodega AND v.estante = _Nombre) THEN
	UPDATE estante SET Nombre=_Nombre,estatus=_estatus,id_bodega=_id_bodega where id_estante=_id_estante;
    SELECT true as RS;
END;
ELSE
	SELECT false as RS;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_modificarProductos_wvp` (IN `_nom` VARCHAR(100), IN `_uni` VARCHAR(50), IN `_idProv` INT, IN `_max` DOUBLE(10,2), IN `_min` DOUBLE(10,2), IN `_marca` VARCHAR(100), IN `_status` CHAR(1), IN `_cto` DOUBLE(10,2), IN `_idCat` INT, IN `_idProc` INT)  NO SQL
UPDATE productos
SET 
	Nombre = _nom ,
    Unidad_medida = _uni,
...
DROP PROCEDURE IF EXISTS `sp_Productos_vendidos`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_Productos_vendidos` ()  NO SQL
select Max(id_producto) as mas,Nombre
from productos
group by Nombre
...
(6, 'Bodega Z', 'A', 'Bodega instalacion', 263);
...
(1, 2, 2, '378.20', '2550.00', 6.74),
(2, 1, 2, '240.00', '160.00', 0.67),
(3, 3, 2, '220.00', '600.00', 2.73),
(4, 3, 1, '150.00', '385.10', 2.57),
(5, 4, 2, '323.30', '1240.00', 3.84),
(6, 2, 1, '160.00', '520.00', 3.25),
(14, 1, 1, '40.00', '100.00', 2.50),
(15, 8, 10, '300.00', '30.00', 0.10),
(16, 1, 10, '25.00', '75.00', 3.00),
(17, 2, 10, '25.00', '75.00', 3.00),
(18, 4, 10, '95.00', '50.00', 0.53),
(19, 3, 10, '100.00', '100.00', 1.00),
(20, 5, 10, '115.00', '110.00', 0.96),
(21, 7, 10, '200.00', '50.00', 0.25),
(22, 5, 5, '90.00', '250.00', 2.78);
...
(55, 1.00, 100.00, 0.00, 0.00, 2, 2, 15),
(56, 2.00, 100.00, 0.00, 0.00, 2, 2, 16),
(57, 10.00, 20.00, 0.00, 0.00, 2, 2, 17),
(58, 40.00, 10.00, 0.00, 0.00, 2, 2, 18),
(59, 3.00, 100.00, 0.00, 0.00, 2, 2, 19),
(60, 3.00, 100.00, 0.00, 0.00, 2, 2, 20),
(61, 3.00, 100.00, 0.00, 0.00, 2, 2, 21),
(62, 3.00, 100.00, 0.00, 0.00, 2, 2, 22),
(63, 3.00, 100.00, 0.00, 0.00, 2, 2, 23),
(64, 3.00, 100.00, 0.00, 0.00, 2, 2, 24),
(65, 3.00, 100.00, 0.00, 0.00, 2, 2, 25),
(66, 3.00, 100.00, 0.00, 0.00, 2, 2, 26),
(67, 3.00, 100.00, 0.00, 0.00, 2, 2, 27),
(68, 3.00, 100.00, 0.00, 0.00, 2, 2, 28),
(69, 5.00, 10.00, 0.00, 0.00, 1, 2, 29),
(70, 10.00, 25.00, 0.00, 0.00, 1, 2, 30),
(71, 15.00, 75.00, 0.00, 0.00, 1, 2, 31),
(72, 100.00, 10.00, 0.00, 0.00, 1, 2, 32),
(73, 100.00, 10.00, 0.00, 0.00, 1, 2, 33),
(74, 150.00, 400.00, 0.00, 0.00, 2, 2, 34),
(75, 50.00, 150.00, 0.00, 0.00, 3, 2, 35),
(76, 50.00, 150.00, 0.00, 0.00, 3, 2, 36),
(77, 50.00, 150.00, 0.00, 0.00, 3, 2, 37),
(78, 10.00, 50.00, 0.00, 0.00, 3, 2, 38),
(79, 10.00, 50.00, 0.00, 0.00, 3, 2, 39),
(80, 20.00, 100.00, 0.00, 0.00, 3, 1, 40),
(81, 75.00, 200.00, 0.00, 0.00, 3, 1, 41),
(82, 15.00, 10.00, 0.00, 0.00, 2, 2, 42),
(83, 25.00, 60.00, 0.00, 0.00, 3, 1, 43),
(84, 5.00, 45.00, 0.00, 0.00, 2, 1, 44),
(85, 5.00, 50.00, 0.00, 0.00, 2, 1, 45),
(86, 5.00, 50.00, 0.00, 0.00, 2, 1, 46),
(87, 5.00, 10.00, 0.00, 0.00, 2, 1, 47),
(88, 5.00, 10.00, 0.00, 0.00, 2, 1, 48),
(89, 20.00, 40.00, 0.00, 0.00, 2, 1, 49),
(90, 5.00, 45.00, 0.00, 0.00, 2, 1, 50),
(91, 5.00, 10.00, 0.00, 0.00, 2, 1, 51),
(92, 5.00, 10.00, 0.00, 0.00, 2, 1, 52),
(93, 100.00, 10.00, 0.00, 0.00, 8, 10, 53),
(94, 100.00, 10.00, 0.00, 0.00, 8, 10, 54),
(95, 100.00, 10.00, 0.00, 0.00, 8, 10, 55),
(96, 25.00, 75.00, 0.00, 0.00, 1, 10, 56),
(97, 25.00, 75.00, 0.00, 0.00, 2, 10, 57),
(98, 95.00, 50.00, 0.00, 0.00, 4, 10, 58),
(99, 100.00, 100.00, 0.00, 0.00, 3, 10, 59),
(100, 15.00, 35.00, 0.00, 0.00, 5, 10, 60),
(101, 200.00, 50.00, 0.00, 0.00, 7, 10, 61),
(102, 100.00, 75.00, 0.00, 0.00, 5, 10, 62),
(103, 40.00, 150.00, 0.00, 0.00, 5, 5, 63),
(104, 50.00, 100.00, 0.00, 0.00, 5, 5, 64);
...
(13, '2020-06-07 02:40:57', 'sp_entradaMovimiento_wvp', 0.00, 'A', 1, 1, 1),
(14, '2020-06-07 02:43:47', 'sp_entradaMovimiento_wvp', 0.00, 'A', 1, 1, 1),
(15, '2020-06-07 02:46:11', 'sp_entradaMovimiento_wvp', 100.00, 'A', 1, 1, 1),
(16, '2020-06-07 02:47:23', 'sp_entradaMovimiento_wvp', 100.00, 'A', 1, 1, 1),
(17, '2020-06-07 02:55:57', 'sp_entradaMovimiento_wvp', 20.00, 'A', 1, 1, 1),
(18, '2020-06-07 02:58:01', 'sp_entradaMovimiento_wvp', 10.00, 'A', 1, 1, 1),
(19, '2020-06-07 03:12:57', 'sp_entradaMovimiento_wvp', 100.00, 'A', 1, 1, 1),
(20, '2020-06-07 03:19:29', 'sp_entradaMovimiento_wvp', 100.00, 'A', 1, 1, 1),
(21, '2020-06-07 03:19:47', 'sp_entradaMovimiento_wvp', 100.00, 'A', 1, 1, 1),
(22, '2020-06-07 03:20:33', 'sp_entradaMovimiento_wvp', 100.00, 'A', 1, 1, 1),
(23, '2020-06-07 03:21:09', 'sp_entradaMovimiento_wvp', 100.00, 'A', 1, 1, 1),
(24, '2020-06-07 03:21:35', 'sp_entradaMovimiento_wvp', 100.00, 'A', 1, 1, 1),
(25, '2020-06-07 03:22:48', 'sp_entradaMovimiento_wvp', 100.00, 'A', 1, 1, 1),
(26, '2020-06-07 03:23:09', 'sp_entradaMovimiento_wvp', 100.00, 'A', 1, 1, 1),
(27, '2020-06-07 03:23:49', 'sp_entradaMovimiento_wvp', 100.00, 'A', 1, 1, 1),
(28, '2020-06-07 03:24:26', 'sp_entradaMovimiento_wvp', 100.00, 'A', 1, 1, 1),
(29, '2020-06-07 03:25:20', 'sp_entradaMovimiento_wvp', 10.00, 'A', 1, 1, 1),
(30, '2020-06-07 03:27:11', 'sp_entradaMovimiento_wvp', 25.00, 'A', 1, 1, 1),
(31, '2020-06-07 03:27:57', 'sp_entradaMovimiento_wvp', 75.00, 'A', 1, 1, 1),
(32, '2020-06-07 03:30:45', 'sp_entradaMovimiento_wvp', 10.00, 'A', 1, 1, 1),
(33, '2020-06-07 03:30:45', 'sp_entradaMovimiento_wvp', 10.00, 'A', 1, 1, 1),
(34, '2020-06-07 12:50:35', 'sp_entradaMovimiento_wvp', 400.00, 'A', 1, 1, 1),
(35, '2020-06-07 12:55:16', 'sp_entradaMovimiento_wvp', 150.00, 'A', 1, 1, 1),
(36, '2020-06-07 12:58:36', 'sp_entradaMovimiento_wvp', 150.00, 'A', 1, 1, 1),
(37, '2020-06-07 13:09:48', 'sp_entradaMovimiento_wvp', 150.00, 'A', 1, 1, 1),
(38, '2020-06-07 13:10:10', 'sp_entradaMovimiento_wvp', 50.00, 'A', 1, 1, 1),
(39, '2020-06-07 13:12:31', 'sp_entradaMovimiento_wvp', 50.00, 'A', 1, 1, 1),
(40, '2020-06-07 14:53:53', 'sp_entradaMovimiento_wvp', 100.00, 'A', 1, 1, 1),
(41, '2020-06-07 14:55:18', 'sp_entradaMovimiento_wvp', 200.00, 'A', 1, 1, 1),
(42, '2020-06-07 15:00:39', 'sp_entradaMovimiento_wvp', 10.00, 'A', 1, 1, 1),
(43, '2020-06-07 15:19:54', 'sp_entradaMovimiento_wvp', 60.00, 'A', 1, 1, 1),
(44, '2020-06-07 15:21:49', 'sp_entradaMovimiento_wvp', 45.00, 'A', 1, 1, 1),
(45, '2020-06-07 15:39:55', 'sp_entradaMovimiento_wvp', 50.00, 'A', 1, 1, 1),
(46, '2020-06-07 15:39:59', 'sp_entradaMovimiento_wvp', 50.00, 'A', 1, 1, 1),
(47, '2020-06-07 15:40:41', 'sp_entradaMovimiento_wvp', 10.00, 'A', 1, 1, 1),
(48, '2020-06-07 15:41:22', 'sp_entradaMovimiento_wvp', 10.00, 'A', 1, 1, 1),
(49, '2020-06-07 15:41:48', 'sp_entradaMovimiento_wvp', 40.00, 'A', 1, 1, 1),
(50, '2020-06-07 15:45:13', 'sp_entradaMovimiento_wvp', 45.00, 'A', 1, 1, 1),
(51, '2020-06-07 15:49:59', 'sp_entradaMovimiento_wvp', 10.00, 'A', 1, 1, 1),
(52, '2020-06-07 15:50:21', 'sp_entradaMovimiento_wvp', 10.00, 'A', 1, 1, 1),
(53, '2020-06-10 20:48:40', 'sp_entradaMovimiento_wvp', 10.00, 'A', 6, 1, 1),
(54, '2020-06-10 20:48:55', 'sp_entradaMovimiento_wvp', 10.00, 'A', 6, 1, 1),
(55, '2020-06-10 21:08:04', 'sp_entradaMovimiento_wvp', 10.00, 'A', 6, 1, 1),
(56, '2020-06-10 21:27:58', 'sp_entradaMovimiento_wvp', 75.00, 'A', 6, 1, 1),
(57, '2020-06-10 21:29:23', 'sp_entradaMovimiento_wvp', 75.00, 'A', 6, 1, 1),
(58, '2020-06-10 21:35:23', 'sp_entradaMovimiento_wvp', 50.00, 'A', 6, 1, 1),
(59, '2020-06-10 21:40:30', 'sp_entradaMovimiento_wvp', 100.00, 'A', 6, 1, 1),
(60, '2020-06-10 21:44:58', 'sp_entradaMovimiento_wvp', 35.00, 'A', 6, 1, 1),
(61, '2020-06-10 21:46:51', 'sp_entradaMovimiento_wvp', 50.00, 'A', 6, 1, 1),
(62, '2020-06-10 22:29:46', 'sp_entradaMovimiento_wvp', 75.00, 'A', 6, 1, 1),
(63, '2020-06-12 01:31:52', 'sp_entradaMovimiento_wvp', 150.00, 'A', 2, 1, 1),
(64, '2020-06-12 01:32:26', 'sp_entradaMovimiento_wvp', 100.00, 'A', 2, 1, 1);
...
(1, 'martillo ', 'martillo.jpg', 'uni', 1, 5, 20, 'truper', 'A', 2.06, 1),
(2, 'serrucho', 'serrucho.jpg', 'uni', 2, 5, 20, 'trump', 'A', 4.33, 1),
(3, 'baterias', 'batarias.jpg', 'uni', 2, 20, 300, 'gold', 'A', 2.10, 1),
(4, 'Esmeril', 'esmaril.jpg', 'uni', 1, 30, 200, 'Trump', 'A', 2.19, 1),
(5, 'Piocha', 'piocha.jpg', 'uni', 2, 5, 200, 'truper', 'A', 1.87, 2),
(7, 'ladrillos', 'producto.jpg', 'uni', 1, 2000, 10, 'varios', 'A', 0.25, 2),
(8, 'Clavos', 'producto.jpg', 'libras', 2, 2000, 20, 'varios', 'A', 0.10, 2);
...
--
-- Estructura de tabla para la tabla `temp_movimientos`
--
...
--
-- Volcado de datos para la tabla `temp_movimientos`
--

INSERT INTO `temp_movimientos` (`id_temp`, `id_producto`, `id_nivel`, `cantidad`, `costo`, `id_usuario`, `id_orden`) VALUES
(1, 1, 1, 12, 12, 1, 1),
(2, 2, 1, 1, 6.74, 1, 4),
(3, 2, 1, 1, 6.74, 1, 4),
(4, 2, 1, 1, 6.74, 1, 4),
...
(8, 2, 2, 2, 13.48, 1, 4),
(9, 1, 1, 1, 0.67, 1, 4),
(10, 5, 12, 12, 11.52, 1, 4),
(11, 5, 12, 12, 11.52, 1, 4);
...
(49, 'Jorge Gavidia', 'jorge.gavidia', '202cb962ac59075b964b07152d234b70', 'foto1.jpg', 'A', 2);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_concepto_wvp`  AS  select `c`.`id_concepto` AS `id_concepto`,`c`.`concepto` AS `concepto`,`c`.`id_tipo_concepto` AS `id_tipo_concepto`,`t`.`tipo_concepto` AS `tipo_concepto` from (`concepto` `c` join `tipo_conceptos` `t` on((`t`.`id_tipo_concepto` = `c`.`id_tipo_concepto`))) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_estantes_wvp`  AS  select `e`.`id_estante` AS `id_estante`,`b`.`id_bodega` AS `ID`,`b`.`Nombre` AS `bodega`,`e`.`Nombre` AS `estante`,`e`.`estatus` AS `letra_status`,`fn_status_wvp`(`e`.`estatus`) AS `estatus`,count(`n`.`id_nivel`) AS `num_niveles` from ((`estante` `e` left join `bodega` `b` on((`e`.`id_bodega` = `b`.`id_bodega`))) left join `nivel` `n` on((`n`.`id_estante` = `e`.`id_estante`))) group by `n`.`id_estante` order by `b`.`Nombre`,`e`.`Nombre` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_nivel_wvp`  AS  select `n`.`id_nivel` AS `id_nivel`,`n`.`Nivel` AS `Nivel`,`n`.`estatus` AS `estatus`,`b`.`id_bodega` AS `id_bodega`,`b`.`Nombre` AS `bodega`,`n`.`id_estante` AS `id_estante`,`e`.`Nombre` AS `estante` from ((`nivel` `n` join `estante` `e` on((`e`.`id_estante` = `n`.`id_estante`))) join `bodega` `b` on((`b`.`id_bodega` = `e`.`id_bodega`))) order by `b`.`Nombre`,`e`.`Nombre`,`n`.`Nivel` ;
...
--
-- Indices de la tabla `temp_movimientos`
--
ALTER TABLE `temp_movimientos`
...
--
ALTER TABLE `estante`
  MODIFY `id_estante` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=8;
...
--
ALTER TABLE `movimientos`
  MODIFY `id_movimiento` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=105;
...
--
ALTER TABLE `ordenes`
  MODIFY `id_orden` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=65;
...
--
-- AUTO_INCREMENT de la tabla `temp_movimientos`
--
ALTER TABLE `temp_movimientos`
